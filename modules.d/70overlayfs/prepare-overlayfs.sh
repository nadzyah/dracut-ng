...
if getargbool 0 rd.live.image -d live_image; then
    overlay=""
    overlayroot=""
else
...
[ -n "$overlayfs" ] || [ -n "$overlay" ] || [ -n "$overlayroot" ] || return 0

overlay_mode="tmpfs"
overlay_device=""

# Process overlayroot= first (cloud-initramfs-tools compatibility)
